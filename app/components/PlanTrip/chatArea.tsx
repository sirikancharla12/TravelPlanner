...
export default function TripPlanner({
  ...
  const handleSavedTrips = async () => {
    ...
    const lastPlan = [...messages]
      .reverse()
      .find((m) => m.role === "assistant" && m.type === "plan");
    ...
    try {
      ...
      if (json?.alreadyExists) {
        alert(
          "This trip already exists in your saved trips. Open it from Saved Trips."
        );
      } else {
        alert("Trip saved!");
      }
  ...
  function getOriginFromLastUserMessage(
    messages: Message[]
  ): string | undefined {
        ...
        <div className="max-w-6xl mx-auto px-6 flex items-center gap-3">
          <textarea
            className="flex-1 p-4 h-16 rounded-2xl border border-gray-300 
              ...
              resize-none text-[var(--color-heading)] placeholder:text-[var(--color-small)]"
            placeholder={
              messages.length
          ...
          <button
            ...
            disabled={loading}
            className="p-4 rounded-2xl bg-[var(--color-primary)] text-white 
              font-semibold shadow hover:shadow-lg transition disabled:opacity-50"
          >
